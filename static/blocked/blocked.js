...
document.querySelector("#visitAnyways").addEventListener("click", async () => {
	...
	browser.runtime.sendMessage({type: "addBlockingException", data: {deathDate: Date.now()+allowedLength, tabId: (await browser.tabs.getCurrent()).id, allowedLength: allowedLength} }).then(res => {
